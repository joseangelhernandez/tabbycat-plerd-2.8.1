...
# to a debate and then sets the ballotsubmission motion field, if null.
    operations = [
        migrations.RunSQL(
            """
            UPDATE bs
            SET motion_id = rm.motion_id
            FROM results_ballotsubmission bs
            INNER JOIN draw_debate d ON d.id = bs.debate_id
            ...
            INNER JOIN (
                SELECT round_id, motion_id
                FROM motions_roundmotion
                WHERE round_id IN (
                    SELECT round_id
                    FROM motions_roundmotion
                    GROUP BY round_id
                    HAVING COUNT(motion_id) = 1
                )
            ) rm ON r.id = rm.round_id
            WHERE bs.motion_id IS NULL
            AND (p.raw_value = 'False' OR p.raw_value IS NULL);
            """,
            """
            UPDATE bs
            SET motion_id = NULL
            ...
            WHERE (p.raw_value = 'False' OR p.raw_value IS NULL);
            """
        ),
    ]
