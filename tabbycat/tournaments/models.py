...
from django.db.models import Count, F, Prefetch, Q, Case, When, Value, IntegerField
...
class Tournament(models.Model):
    ...
    seq = models.IntegerField(default=0,  # Establecer un valor predeterminado no nulo
...
class RoundManager(LookupByNameFieldsMixin, models.Manager):
    ...
    def get_queryset(self):
        # Anota el queryset con un valor de ordenamiento que maneja nulos
        queryset = super().get_queryset().select_related('tournament').annotate(
            sorted_seq=Case(
                When(tournament__seq__isnull=True, then=Value(999999)),  # Uso de un valor alto para nulos
                default='tournament__seq',
                output_field=IntegerField()
            )
        ).order_by('sorted_seq')  # Ordena utilizando el campo anotado que maneja nulos
        return queryset
